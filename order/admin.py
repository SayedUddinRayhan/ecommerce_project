...
class OrderItemAdmin(admin.ModelAdmin):
    list_display = ('order', 'product', 'display_variations', 'quantity', 'price', 'total_price', 'refund_status')
    list_filter = ('order', 'product', 'refund_status')  # Updated to match refund system
    search_fields = ('order__order_id', 'product__product_name')

    actions = ['request_refund', 'approve_return', 'reject_return']
    ...
    def request_refund(self, request, queryset):
        """Allow admins to request a refund for delivered orders."""
        for item in queryset:
            if item.order.status == 'delivered' and item.refund_status == 'not_requested':
                item.refund_status = 'requested'
                item.return_requested_date = now()
                item.save()
                self.message_user(request, f"Refund requested for Order Item {item.id}.")
            else:
                self.message_user(request, f"Refund request failed for Order Item {item.id}. It must be delivered and not already requested.")

    request_refund.short_description = "Request Refund"

    def approve_return(self, request, queryset):
        """Approve the return and increase the product stock."""
        for item in queryset:
            if item.refund_status == 'requested':  # Ensure the return has been requested
    ...
    def reject_return(self, request, queryset):
        """Reject the return."""
        for item in queryset:
            if item.refund_status == 'requested':  # Only allow rejection for 'requested' status
